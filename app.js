document.addEventListener('DOMContentLoaded', () => {
    ...
    // =====================================================================
    // ======= 2. ELEMENT SELECTORS & STATE VARIABLES ======================
    // =====================================================================
    ...
    // =====================================================================
    // ======= 3. AUTHENTICATION & DATA SYNC LOGIC (CORRECTED) =============
    // =====================================================================
    ...
    const saveBookmarks = () => { auth.currentUser ? updateUserDocInFirestore({ bookmarks }) : localStorage.setItem('toolHubBookmarks', JSON.stringify(bookmarks)); };
    ...
    const syncWithFirestore = async (firebaseUser) => {
        ...
        if (!doc.exists) {
            console.log("First time login for this user. Migrating guest data to Firestore.");
    ...
    const handleDataViewClick = (e) => { const link = e.target.closest('a[data-view]'); if (!link) return; e.preventDefault(); const view = link.dataset.view; if (view === 'profile') { if (auth.currentUser) { history.pushState({ view: 'profile' }, '', '/profile'); switchView('profile'); } else { profileSignInModal.classList.add('show'); } return; } const newPath = view === 'home' ? '/' : `/${view}`; history.pushState({ view: view }, '', newPath); switchView(view); };
    ...
    const handleBookmarkClick = (button) => { const toolId = button.dataset.toolId; if (!toolId) return; if (isBookmarked(toolId)) { bookmarks = bookmarks.filter(id => id !== toolId); button.classList.remove('bookmarked'); } else { if (bookmarks.length >= GUEST_BOOKMARK_LIMIT && !auth.currentUser) { showModal(); return; } bookmarks.push(toolId); button.classList.add('bookmarked'); } saveBookmarks(); if (currentView === 'your-tools') renderYourToolsView(); };
